...
- **Connection endpoints are resolved against the set of identities present in that diagram**,
  ...
  repository's own documents. A connection whose endpoints do not resolve is left inert rather
  than attached to the wrong element.

Only the first rule has code today: `deriveDiagramModel` reads elements, which is all the
capability probe needs. Connections are read where neighbourhood highlighting is built, and
the second rule binds that code.
...
- **The capability probe.** `deriveDiagramModel` returns null when a rendered diagram exposes
  ...
  Degradation is total and silent to the reader; there is never a half-interactive state.
