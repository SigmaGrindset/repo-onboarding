...
- Pre-pass script: `.Codex/skills/onboard/prepass.mjs`
- Edge-integrity checker: `.Codex/skills/onboard/edges-check.mjs`
...
```
node .Codex/skills/onboard/prepass.mjs "<repo-path>" --out "<tmp>/onboard-<repo-name>/prepass.json" --pretty
```
...
- **contributorGuide** — evidence-based contributor guardrails, never generic advice:
  - `knownRisks[]` (aim for 3–5) names a concrete sharp edge with `severity`, the real
    `impact`, a practical `mitigation`, and the repo-relative `files` where it lives.
    Ground risks in invariants, transaction boundaries, security assumptions, concurrency,
    generated code, deployment coupling, or other behavior you verified while reading.
  - `changeRoutes[]` (aim for 4–7) answers “where should this kind of change go?” with a
    recognizable `changeType`, one `primaryPath`, any `relatedPaths`, the architectural
    `rationale`, and specific `verification` hints. Name actual ownership boundaries and
    commands from the repo; do not restate the directory tree.
...
2. **Edge integrity** (the schema does NOT enforce this — you must):
   ```
   node .Codex/skills/onboard/edges-check.mjs data/<repo-name>/analysis.json
   ```
...
- [ ] Risks identify verified failure modes, and every change route points to real paths
      with repo-specific verification guidance.
